...
`endif
    ...
    int cycle_cnt = 0;
    always @(posedge clk_i) cycle_cnt <= rst_i ? 0 : cycle_cnt + 1;
    ...
    task automatic sample_outputs();
        @(posedge clk_i iff (output_valid_o === 1'b1));
        for (int m = 0; m < NUM_MAT; m++) begin
            for (int r = 0; r < N; r++) begin
                for (int c = 0; c < N; c++) C_actual[m][r][c] = final_psums_o[c];
                if (!(m == NUM_MAT-1 && r == N-1)) @(posedge clk_i);
            end
            display_results(m);
        end
    endtask
...
module top_mod_tb;
    localparam bit TB_SHADOW_ENABLED = 1'b0;
    localparam int N = 8;
    localparam int DATA_WIDTH = 8;
    localparam int PSUM_WIDTH = 32;
